...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if args.use_multi_supp:
        optimizer = torch.optim.SGD(
            [
                {'params': model.down_query.parameters()},
                {'params': model.down_supp.parameters()},
                {'params': model.theta.parameters()},
                {'params': model.nsm.parameters()},
                ...
                {'params': model.MaskedSuppFeat.parameters()},
                {'params': model.GetAdaptiveProto.parameters()},
            ],
            lr=args.base_lr, momentum=args.momentum, weight_decay=args.weight_decay)
    elif args.use_mid_nsm:
        optimizer = torch.optim.SGD(
            [
                {'params': model.down_query.parameters()},
                {'params': model.down_supp.parameters()},
    ...
    for i, (input, target, s_input, s_mask, subcls) in enumerate(train_loader):
        ...
        if (i + 1) % args.print_freq == 0 and main_process():
            logger.info('Epoch: [{}/{}][{}/{}] '
                        ...
                        'AuxLoss {aux_loss_meter.val:.4f} ({aux_loss_meter.avg:.4f}) '
                        'Loss {loss_meter.val:.4f} {loss_meter.avg:.4f} '
                        'Accuracy {accuracy_meter.val:.4f} ({accuracy_meter.avg:.4f}).'.format(epoch + 1, args.epochs,
...
def validate(val_loader, model, criterion):
    print(f"=====> 真实加载的验证集图片数量: {len(val_loader.dataset)}")
    if main_process():
