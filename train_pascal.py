...
def get_parser():
    ...
    parser.add_argument('--arch', type=str, default='HMNet')  #
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # ----------------------  TRAIN  -----------------------
        # === [新增 AMP 逻辑 2/4] 把 scaler 传进去 ===
        loss_train, mIoU_train, mAcc_train, allAcc_train = train(train_loader, val_loader, model, optimizer, optimizer_swin, epoch, scaler)
        ...
        # -----------------------  VAL  ------------------------
        if args.evaluate and epoch % 1 == 0 and epoch > 25:
            ...
            # === [新增画图逻辑 3/4] 每轮验证后追加数据 ===
            if main_process():
                plot_epochs.append(epoch_log)
                plot_train_losses.append(loss_train)
                plot_val_losses.append(loss_val)
                plot_val_mious.append(mIoU)
                plot_val_fbious.append(FBIoU)
            # ============================================

    # 注意：下面这行是你原本的代码，标志着 for epoch 循环结束
    total_time = time.time() - start_time
    
    # === [新增画图逻辑 4/4] 循环结束后统一绘制并保存图表 ===
    if main_process() and len(plot_epochs) > 0:
        logger.info(">>>>>>>>>>>>>>>> 正在生成并保存训练指标图表 >>>>>>>>>>>>>>>>")
        # 尽量使用学术图表风格
        plt.style.use('default') 
        
        # 1. 绘制 Loss 对比曲线 (Train vs Val)
        plt.figure(figsize=(8, 6))
        plt.plot(plot_epochs, plot_train_losses, label='Train Loss', color='#1f77b4', linewidth=2)
        plt.plot(plot_epochs, plot_val_losses, label='Val Loss', color='#d62728', linestyle='--', linewidth=2)
        plt.xlabel('Epoch', fontsize=14)
        plt.ylabel('Loss', fontsize=14)
        plt.title('Training and Validation Loss', fontsize=16)
        plt.legend(fontsize=12)
        plt.grid(True, linestyle=':', alpha=0.7)
        plt.savefig(osp.join(args.result_path, 'loss_curve.png'), dpi=300, bbox_inches='tight')
        plt.savefig(osp.join(args.result_path, 'loss_curve.pdf'), bbox_inches='tight') # 无损矢量图
        plt.close()

        # 2. 绘制 mIoU 和 FBIoU 趋势图
        plt.figure(figsize=(8, 6))
        plt.plot(plot_epochs, plot_val_mious, label='Val mIoU', color='#2ca02c', linewidth=2, marker='o', markersize=4)
        plt.plot(plot_epochs, plot_val_fbious, label='Val FBIoU', color='#9467bd', linewidth=2, marker='^', markersize=4)
        plt.xlabel('Epoch', fontsize=14)
        plt.ylabel('IoU Score', fontsize=14)
        plt.title('Validation Evaluation Metrics', fontsize=16)
        plt.legend(fontsize=12)
        plt.grid(True, linestyle=':', alpha=0.7)
        plt.savefig(osp.join(args.result_path, 'iou_metrics_curve.png'), dpi=300, bbox_inches='tight')
        plt.savefig(osp.join(args.result_path, 'iou_metrics_curve.pdf'), bbox_inches='tight')
        plt.close()
        logger.info(f"论文图表已保存至: {args.result_path}")
    # ========================================================
...
def train(train_loader, val_loader, model, optimizer, optimizer_swin, epoch, scaler):
    ...
    for i, (input, target, s_input, s_mask, subcls) in enumerate(train_loader):
        ...
        from torch.cuda.amp import autocast # 确保当前作用域有 autocast
        
        optimizer.zero_grad()
        optimizer_swin.zero_grad()

        # === [新增 AMP 逻辑 4/4] 开启自动混合精度上下文 ===
        with autocast():
        ...
        # 使用 scaler 放大梯度，反向传播，然后缩放回来更新权重
        ...
        # -----------------------  SubEpoch VAL  ------------------------
        if args.evaluate and args.SubEpoch_val and ((args.epochs <= 100 and epoch % 1 == 0 and epoch > 25) or (epoch > 100)) and (
