...
def make_dataset(split=0, data_root=None, data_list=None, sub_list=None):    
    ...
    for l_idx in tqdm(range(len(list_read))):
       ...
       # label = cv2.imread(label_name, cv2.IMREAD_GRAYSCALE)
        label = np.array(Image.open(label_name))#修改后的
...
class SemData(Dataset):
    ...
    def __getitem__(self, index):
        ...
        label = np.array(Image.open(label_path))  # 新加的
        ...
        for k in range(self.shot):  
            ...
            # 原代码: support_label = cv2.imread(support_label_path, cv2.IMREAD_GRAYSCALE)
            support_label = np.array(Image.open(support_label_path))  # <--- 替换成这行
