...
def denormalize_img(img_tensor):
    """将 Query 图像反归一化并转为 BGR 格式保存"""
    mean = np.array([0.485, 0.456, 0.406]) * 255.0
    std = np.array([0.229, 0.224, 0.225]) * 255.0
    # Tensor转Numpy: (C, H, W) -> (H, W, C)
    img = img_tensor.cpu().numpy().transpose(1, 2, 0)
    img = img * std + mean
    img = np.clip(img, 0, 255).astype(np.uint8)
    img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR) # OpenCV 默认 BGR
    return img

def get_color_mask(mask_tensor):
    """将单通道 Mask 转为红色伪彩色 (忽略类为灰色)"""
    mask = mask_tensor.cpu().numpy()
    color_mask = np.zeros((mask.shape[0], mask.shape[1], 3), dtype=np.uint8)
    color_mask[mask == 1] = [0, 0, 255]     # 前景预测为红色 (BGR格式)
    color_mask[mask == 255] = [128, 128, 128] # 忽略区域(边框等)设为灰色
    return color_mask
# ==================================================
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    # 注意：先把下面这行 DataParallel 注释掉或删掉，我们等加载完权重再包装
    ...
    if args.weight:
        if os.path.isfile(args.weight):
            ...
            # 将修正后的权重加载进模型 (strict=False 可以容忍极个别无关紧要的键值丢失)
    ...
    # 加载完完美的权重后，再统一放到 GPU 里用 DataParallel 包装
    model = torch.nn.DataParallel(model.cuda())
...
def validate(val_loader, model, criterion):
    ...
    for e in range(20):
        for i, (input, target, s_input, s_mask, subcls, ori_label) in enumerate(val_loader):
            ...
            output = output.max(1)[1] # 你原有的代码，获取预测类别

            # ========================== 可视化代码块 ==========================
            # 建议：测试集通常有上千张，为了不撑爆硬盘，可以设置限制（例如只保存前 100 张，或者每 10 张存 1 张）
            # 这里默认保存所有图像，存放在 './vis_results' 文件夹下
            vis_dir = './vis_results'
            os.makedirs(vis_dir, exist_ok=True)
            
            # 当前的类别 ID
            current_cls = subcls[0].item()

            # 1. 还原 Query 图像 (取 batch 中的第一张)
            img_bgr = denormalize_img(input[0])
            
            # 由于可能进行了 Resize 或 backmask 填充，确保图像和 Mask 尺寸一致
            h, w = target[0].shape
            img_bgr = cv2.resize(img_bgr, (w, h), interpolation=cv2.INTER_LINEAR)

            # 2. 获取 GT Mask 和预测 Mask 并上色
            gt_mask_color = get_color_mask(target[0])
            pred_mask_color = get_color_mask(output[0])

            # 3. 将 Mask 半透明叠加到原图上 (alpha=0.6, beta=0.4)
            gt_blend = cv2.addWeighted(img_bgr, 0.6, gt_mask_color, 0.4, 0)
            pred_blend = cv2.addWeighted(img_bgr, 0.6, pred_mask_color, 0.4, 0)

            # 4. 拼图对比: [原图 | 真值 GT | 模型预测 Pred]
            concat_img = np.concatenate([img_bgr, gt_blend, pred_blend], axis=1)

            # 5. 保存图片
            save_name = f'cls_{current_cls}_iter_{iter_num}.jpg'
            cv2.imwrite(os.path.join(vis_dir, save_name), concat_img)
            # ========================== 可视化代码块结束 =======================
